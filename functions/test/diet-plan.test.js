...
test('revisioni: schema 3 con piano, 1/2 invariati, checksum dedicato', () => {
  ...
  const rules = [{ guideFamilyId: 'cereali', ingredientIds: [], quantityGrams: { lunch: { training: 80, rest: 60 }, dinner: null }, enabled: true, categoryId: null }];
...
test('regole vuote ammesse solo con piano guidato (strutture descrittive)', () => {
  ...
  assert.throws(() => validateDietStructureRules([], {}), /tra 1 e 40/);
  const rule = { guideFamilyId: 'cereali', ingredientIds: [], quantityGrams: { lunch: { training: 80, rest: 60 }, dinner: null }, enabled: true, categoryId: null };
