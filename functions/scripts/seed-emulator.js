...
(async () => {
  ...
  const rules = Domain.GUIDE_GRAMMATURE.map(rule => ({
  ...
  const extract = JSON.parse(require('fs').readFileSync(require('path').join(__dirname, '../../docs/catalogo-ingredienti.json'), 'utf8'));
  ...
  batch.set(db.doc(`organizations/${orgId}/dietStructures/struttura-base`), { schemaVersion: 1, name: 'Struttura dieta base', status: 'active', ownerUid: nutritionist.uid, createdBy: nutritionist.uid, currentRevisionId: '1', latestChecksum: structureChecksum, ruleCount: seed.structureSeed.rules.length, ingredientCatalogVersion: 1, createdAt: now, updatedAt: now });
  batch.set(db.doc(`organizations/${orgId}/dietStructures/struttura-base/revisions/1`), { schemaVersion: 1, revisionId: '1', structureId: 'struttura-base', rules: seed.structureSeed.rules, alternativeGroups: seed.structureSeed.alternativeGroups, status: 'published', checksum: structureChecksum, compatibleClientSchema: 6, effectiveAt: now, changelog: 'Seed emulator da split linee guida', createdAt: now, updatedAt: now, createdBy: nutritionist.uid, publishedAt: now, publishedBy: nutritionist.uid });
