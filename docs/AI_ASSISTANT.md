...
- **ascolto con debounce di fine eloquio**: i risultati finali del riconoscimento del browser vengono accumulati in un buffer e processati solo dopo ~1,1 s di silenzio (`RECOGNITION_SILENCE_MS` in `js/assistant.js`, override con `recognitionSilenceMs` in `js/assistant-config.js`). Una pausa a metà frase ("cosa devo mangiare oggi… a pranzo") non produce più un intento troncato; gli errori ASR `aborted`/`no-speech` riavviano il riconoscimento in silenzio. Con `debugRecognition: true` in `js/assistant-config.js` un toast mostra il testo riconosciuto;
...
- **disambiguazione del pasto**: una domanda su un pasto specifico ("cosa mangio stasera", "cosa devo mangiare oggi a pranzo") risponde con il solo pasto richiesto (`get_meal_details`); se manca il pasto ("cosa devo mangiare oggi") l'assistente chiede in locale "per quale pasto? colazione, spuntino, pranzo, merenda o cena?", senza aprire Gemini Live. Alias riconosciuti: mezzogiorno→pranzo, pomeriggio→merenda, sera/stasera→cena, weekend/fine settimana→sabato, "lunedì prossimo"→lunedì. Anche il `systemInstruction` di Gemini Live impone `get_meal_details` per il singolo pasto e riserva `get_current_plan` all'intera giornata o settimana;
