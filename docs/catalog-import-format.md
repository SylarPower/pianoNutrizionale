...
> **Stato**: il callable `importGlobalIngredientCatalog` e le fixture sono introdotti con la stessa revisione dello schema 6; l'esecuzione in produzione resta dietro `CATALOG_IMPORT_ENABLED=false` finché il catalogo definitivo del dott. Meller non sarà approvato. In attesa, il seed deriva **solo** da `MELLER_GRAMMATURE`/estratto autorevole (`splitMellerSeed`).
